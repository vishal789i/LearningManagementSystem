...
<body>
      ...
      <div class="w3-container">
        ...
        <form action="" method="post" id="createStudentForm">
          <div class="w3-section">
            ...
            <input class="w3-input w3-border w3-margin-bottom" type="text" placeholder="Enter Username eg. E2KXXXXXXXX" name="createStudentUserName" pattern="(?=.*[A-Z]).{11,11}" title="Please Enter Your 11 digit Registration ID e.g. E2K1XXXXXXX"
              required>
            <label>Password</label>
            <input class="w3-input w3-border w3-margin-bottom" type="password" placeholder="Enter Password" name="createStudentPassword" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
              title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
            <label>Confirm Password</label>
            <input class="w3-input w3-border w3-margin-bottom" type="password" placeholder="Enter Password" name="createStudentConfirmPassword" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
              title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
  ...
  <!admin sign up>
        ...
        <div class="w3-container">
          ...
          <form action="" method="post" id="createAdminForm">
            <div class="w3-section">
              ...
              <label>Password</label>
              <input class="w3-input w3-border w3-margin-bottom" type="password" placeholder="Enter Password" name="createAdminPassword" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
                title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
              <label>Confirm Password</label>
              <input class="w3-input w3-border w3-margin-bottom" type="password" placeholder="Enter Password" name="createAdminConfirmPassword" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
                title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
      ...
      <div id="start-modal-stusignin" class="w3-modal">
        <div class="w3-modal-content">
          ...
          <div class="w3-container">
			...
			$sqlSubjects = "SELECT examName, subject FROM adminvariables";
			...
			while($row = mysqli_fetch_assoc($resultSubjects)) {
					$subjectsArray[$countSubject++] = $row["subject"];
					$examNameArray[$countExamName++] = $row["examName"];
			}
			
			$lengthSubject = count($subjectsArray);
            ...
            <form action="frontPagePHP.php" method="post">
              <div class="w3-section">
                ...
                <select name="subject">
				<?php for ($i = 0; $i <= $lengthSubject-1; $i++) {?>
                  <option value="<?php echo $subjectsArray[$i]; ?>"> <?php echo $examNameArray[$i]; ?> </option>
				<?php }?>
                </select>
	...
	if( isset($_POST['studentCreateFormButton']) ){
        ...
        <script>
          swal("", "Please re-enter correct confirm password", "warning");
        </script>
        <?php
		}
	}
